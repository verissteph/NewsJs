...
let containerDeNoticiasTecnologia = document.querySelector('#listaDeNoticiasTecnologia')
async function pegarNoticiasTecnologia() {

    let noticiasTecnologia = await fetch('http://newsapi.org/v2/top-headlines?country=br&category=technology&apiKey=82e74070f02b4a2498324d600607c30f');
    let listaDeNoticiasTecnologia = await noticiasTecnologia.json();

    listaDeNoticiasTecnologia.articles.forEach((dados) => {
        let coluna = document.createElement('div');
        coluna.setAttribute('class', 'col-3');
        ...
        containerDeNoticiasTecnologia.appendChild(coluna)
    ...
    console.log(listaDeNoticiasTecnologia);
}